...
const SUPPORTED_EXTENSIONS = ['.md', '.markdown', '.txt', '.epub'];
...
// Open file by path (same format as dialog:openFile)
...
ipcMain.handle('export:pdf', async (event, options = {}) => {
  ...
  try {
    ...
    // Save HTML to temp file (avoids data URL length limits)
    const tempPath = path.join(app.getPath('temp'), `cygnus-pdf-${Date.now()}.html`);
    fs.writeFileSync(tempPath, html, 'utf-8');
    logger.info('PDF export: HTML saved to:', tempPath);
...
ipcMain.handle('export:html', async (event, options = {}) => {
  ...
  try {
    ...
    // Create assets folder and save images
    if (assets.length > 0) {
      ...
      for (const asset of assets) {
        ...
        if (base64Match) {
          ...
          const relativePath = `${path.basename(assetsDir)}/${asset.name}`;
          processedHtml = processedHtml.split(asset.data).join(relativePath);
        }
      }
    }
